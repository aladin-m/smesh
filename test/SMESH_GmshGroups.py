...
with tempfile.TemporaryDirectory() as tmp_dir:
    ...
    try:
      Mesh_1.ExportMESHIO(mesh_file, 'Gmsh 4.0 (*.msh)', Mesh_1)
    except:
      raise Exception("ExportPartToMESHIO() failed. Invalid file name?")
    # reimport file to ensure it can be read back
    ([Mesh_1_reimported], status) = smesh.CreateMeshesFromMESHIO(mesh_file, "Gmsh 4.0 (*.msh)")
    Mesh_1_groups = Mesh_1_reimported.GetGroups()
    for group in Mesh_1_groups:
      assert group.GetName().strip() in ['Group_Vertices', 'Group_0DElements', 'Group_Volume', 'Group_Faces', 'Group_Edges']
    ...
    try:
        ...
        expected_group_names = ['Group_Vertices', 'Group_0DElements', 'Group_Volume', 'Group_Faces', 'Group_Edges']
